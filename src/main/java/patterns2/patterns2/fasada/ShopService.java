...
public class ShopService {
    ...
    public Long openOrder(Long userId){
        if(authenticator.isAuthenticated(userId)){
            Long maxOrder = (long)orders.stream().mapToInt(x->x.getOrderId().intValue()).max().orElse(0);
            orders.add(new Order(maxOrder+1,userId,productService));
            return maxOrder + 1;
        } else return -1L;
    }
    public void addItem(Long orderId, Long productId, double qty){
        orders.stream().filter(x->x.getOrderId().equals(orderId)).forEach(x->x.getItems().add(new Item(productId, qty)));
    }
    public boolean removeItem(Long orderId, Long productId){
        Iterator<Order> orderIterator = orders.stream().filter(x->x.getOrderId().equals(orderId)).iterator();
    ...
    public boolean verifyOrder(Long orderId){
        ...
        while(orderIterator.hasNext()){
            ...
            Random gen = new Random();
            if(!order.isVeryfied()){
                order.setVeryfied(result && gen.nextBoolean());
            }
            return  order.isVeryfied();
        }
        return false;
    }
